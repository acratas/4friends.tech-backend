...
const run = async (db) => {
  const url = `${process.env.BLOCKSCOUT_URL}/addresses/${process.env.CONTRACT_ADDRESS}/logs`;
  ...
  do {
    ...
    for (let log of response.data.items) {
      try {
        await db.users.setFromLog(log);
      } catch (e) {
        console.error(e)
      }
      try {
        await db.transactions.setFromLog(log);
      } catch (e) {
        console.error(e)
      }
    }
